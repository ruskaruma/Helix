...
public:
    ...
    virtual void computeBatch(const float* queries,const float* database,idx_t nq,idx_t nb,dim_t dim,float* distances) const=0;
};

class L2Distance : public DistanceComputer {
public:
    float compute(const float* a,const float* b,dim_t dim) const override;
    void computeBatch(const float* queries,const float* database,idx_t nq,idx_t nb,dim_t dim,float* distances) const override;
